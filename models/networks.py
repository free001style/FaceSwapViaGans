...
from models.encoders.psp_encoders import FSEncoder_PSP
...
class Net3(nn.Module):
    """ Multiscale Style Extraction + StyleGAN with mask-guided injection """
        ...
        for i in range(self.opts.num_seg_cls):
            self.MLPs.append(
                LocalMLP(
                    ...
                    dim_style=512,
                    num_w_layers=18  # do mask injection on all layers
    ...
    def forward(self, img, mask, resize=False, randomize_noise=True, return_latents=False):
        ...
        if self.opts.start_from_latent_avg:
            if self.opts.learn_in_w:
                codes = codes + self.latent_avg[:18, :].repeat(codes.shape[0], codes.shape[1], 1)
                remaining_codes = self.latent_avg[18:, :].repeat(bs, num_comp, 1)
                codes = torch.cat([codes, remaining_codes], dim=2)
            else:
                if self.remaining_layer_idx != 17:
                    codes = codes + self.latent_avg[:18, :].repeat(codes.shape[0], codes.shape[1],
                                                                   1, 1)
                    remaining_codes = self.latent_avg[18:, :].repeat(bs, num_comp, 1, 1)
                    codes = torch.cat([codes, remaining_codes], dim=2)
                else:
