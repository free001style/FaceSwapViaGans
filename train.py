...
from datasets.dataset import CelebAHQDataset, TO_TENSOR, NORMALIZE, FFHQDataset, MASK_CONVERT_TF_DETAILED, \
    FFHQ_MASK_CONVERT_TF_DETAILED
from criteria.id_loss import IDLoss
from criteria.face_parsing.face_parsing_loss import FaceParsingLoss
from criteria.lpips.lpips import LPIPS
from criteria.adverserial_loss import AdvDLoss, AdvGLoss, DR1Loss, GPathRegularizer
from criteria.eye_loss import EyeLoss
...
class Trainer:
    ...
    def __init__(self, opts):
        ...
        if self.opts.eye_detector_lambda > 0:
            self.eye_detector_loss = EyeLoss().to(self.device).eval()
        ...
        if self.rank == 0:
            ...
            wandb.login(key=KEY)
            wandb.init(project='course_project', entity='free001style', name=opts.exp_dir, config=self.opts)
        ...
        while self.global_step <= self.opts.max_steps:
            for batch_idx, batch in enumerate(self.train_dataloader):
                ...
                if self.opts.train_D and (self.global_step % self.opts.d_every == 0):
                    ...
                    recon, _, latent = self.net(img, onehot, return_latents=True)
                    fake_pred = self.D(recon)
                ...
                loss_, loss_dict = self.calc_loss(img, recon)
                ...
                self.global_step += 1
                if self.global_step == self.opts.max_steps // 2:
    ...
    def calc_loss(self, img, recon1):
        loss_dict = {}
        loss = 0.0

        if self.opts.eye_detector_lambda > 0:
            loss_eye = self.eye_detector_loss(recon1, img)

            loss_dict['loss_eye_detector'] = float(loss_eye)
            loss += self.opts.eye_detector_lambda * loss_eye

        if self.opts.face_parsing_lambda > 0:
            loss_face_parsing = self.face_parsing_loss(recon1, img)

            loss_dict['loss_face_parsing'] = float(loss_face_parsing)
            loss += loss_face_parsing * self.opts.face_parsing_lambda

        if self.opts.id_lambda > 0:
            loss_id = self.id_loss(recon1, img)

            loss_dict['loss_id'] = float(loss_id)
            loss += loss_id * self.opts.id_lambda

        if self.opts.l2_lambda > 0:
            loss_l2 = F.mse_loss(recon1, img)
    ...
    @staticmethod
    def parse_images(mask, img, recon1, display_count=2):
        im_data = []
    ...
    def log_images(self, name, imgs1_data, subscript=None, log_latest=False):
        ...
        #     wandb.log({f'images_11_stylegan_all_inject/train/{step:06d}.jpg':wandb.Image(path)},step=self.global_step)
        # elif 'test' in name:
        #     self.logger.add_figure(
        #         f'test/{step:06d}.jpg', fig, self.global_step)
        #     wandb.log({f'images_11_stylegan_all_inject/test/{step:06d}.jpg':wandb.Image(path)}, step=self.global_step)
